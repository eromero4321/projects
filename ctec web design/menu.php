...
//send the query to the database and get results
    ...
    */
	$sql = "SELECT CategoryName, CID FROM CasaAzulCategory order by CategoryName ASC ;";
	...
	if ($stmt->prepare($sql)) {
		...
		$stmt->bind_result($CategoryName, $CID);

		echo "<nav>";
		/* fetch values */
		while ($stmt->fetch()) {
			echo "| <a href='menu.php?CID=$CID'>$CategoryName </a>";
		}
		echo "</nav>";
...
/* close connection */
$CID= $_GET['CID'];
	$sql = "SELECT `LinkID`, `PictureURL`,`Price`, `MenuItem`,`Description` , `CID` FROM CasaAzulMenu WHERE CID= ?;";
    ...
    $stmt->bind_param('i', $CID);
		...
		/* bind result variables */
		$stmt->bind_result($LinkID, $PictureURL,$Price, $MenuItem, $Description , $CID);
        
        $hasResults= false;
		
		/* fetch values */
		while ($stmt->fetch()) {
			echo "<ul> <img src='$PictureURL' width= '150' alt='Picture'><br/>$MenuItem   $$Price<br/> $Description <br/></a><br/></ul>\n";
			$hasResults= true;
		}
