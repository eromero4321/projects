...
if (isset($_GET['LinkID'])) { // note that the spelling 'pid' is based on the query string variable name
	...
	// validate the product id -- check to see if it is greater than 0
		if ($LinkID>0 ){
			//compose the query
			$sql = "DELETE from peoplesreviews WHERE LinkID = ?"; // note that the spelling "PID" is based on the field name in the database product table.
...
<main class='flexboxContainer'>
    ...
    <div>   
        ...
        <p>Back to the <a href='admin_productList2.php'>product list page</a></p>
    </div>
