...
if (isset($_POST['Submit'])) {
	...
	$required = array("MenuItem", "Price","PictureURL", "Description", "CID"); // note that, in this array, the spelling of each item should match the form field names

	// set up the expected array
	$expected = array("MenuItem", "Price", "CID", "PictureURL","Description"); // again, the spelling of each item should match the form field names
    
    // set up a label array, use the field name as the key and label as the value
    $label = array ("MenuItem"=>'Menu Item', "Price"=>'Price', "CID"=>'Drink Category', "PictureURL"=>'Picture',"Description"=>'Description');
	...
	if (empty($missing)){
		...
		if ($LinkID != "") {
			...
			$sql = "Update CasaAzulMenu SET MenuItem = ?, Price = ?, PictureURL= ?, Description = ?, CID = ? WHERE LinkID = ?";
				
			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.
				$stmt->bind_param('sdssi',$MenuItem, $Price, $PictureURL, $Description, $CID );
			...
			$sql = "Insert Into CasaAzulMenu (MenuItem, Price, PictureURL, Description, CID) values (?, ?, ?, ?, ?)";
...
echo $PictureURL.'<br>';
	...
	$output = "<div class='error'>Please begin your product managment operation from the <a href='admin_productList.php'>admin page</a>.</div>";
}
